    ...
    pub fn set_symbol_value(&mut self, value: SymbolValue) -> Result<()> {
        ...
        match value {
            SymbolValue::Boolean(value) => {
                ...
                ensure!(ret, format!("TODO Could not set symbol {:?}", self.name()))
            }
            SymbolValue::Tristate(value) => {
                // TODO if boolean then mod is not allowed
                ensure!(
                    self.symbol_type() == SymbolType::Tristate || self.symbol_type() == SymbolType::Boolean,
                    format!("TODO, not tristate, but {:?}", self.symbol_type())
                );
                ensure!(!self.is_const(), "TODO, const");
